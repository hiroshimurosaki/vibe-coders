<h1 align="center">🏆 Vibe Coders</h1>

<p align="center">
  <em>Programação Competitiva</em><br>
  Extensão <strong>PROTIVA</strong> · <strong>UNESP — Bauru</strong>
</p>

<!-- Coloque a foto do time em assets/team.jpg (ou troque a extensão abaixo) -->
<p align="center">
  <img src="assets/team.jpg" alt="Foto do time Vibe Coders" width="520">
</p>

---

## 👥 O Time

Somos o **Vibe Coders**, um time de programação competitiva da extensão
**PROTIVA**, da **UNESP — campus de Bauru**. Treinamos resolvendo problemas no
Codeforces e no CSES e participamos de competições como a Maratona SBC de
Programação (ICPC Brasil), a Paulista e a Mineira.

<!-- PREENCHER: nomes, curso/ano e handles dos integrantes -->
| Integrante | Curso / Ano | Codeforces | GitHub |
|---|---|---|---|
| _(nome)_ | _(curso, ano)_ | [@handle](https://codeforces.com/profile/handle) | [@user](https://github.com/user) |
| _(nome)_ | _(curso, ano)_ | [@handle](https://codeforces.com/profile/handle) | [@user](https://github.com/user) |
| _(nome)_ | _(curso, ano)_ | [@handle](https://codeforces.com/profile/handle) | [@user](https://github.com/user) |

---

## 📊 Estatísticas

> Atualizadas automaticamente a cada push pelo workflow
> [`update-stats`](.github/workflows/update-stats.yml).

<!-- STATS:START -->
> **118** problemas resolvidos · **41** Codeforces · **37** CSES · **40** em provas (10 competições)

### Por fonte

```mermaid
pie showData
    title Problemas por fonte
    "Codeforces" : 41
    "CSES" : 37
    "Provas" : 40
```

### Codeforces por tema

```mermaid
pie showData
    title Codeforces por tema
    "Greedy" : 15
    "DP" : 6
    "Strings" : 6
    "Implementation" : 4
    "Math" : 4
    "Constructive" : 2
    "Geometry" : 2
    "Data Structures" : 1
    "Number Theory" : 1
```

| Tema | Resolvidos |
|---|---|
| Greedy | 15 |
| DP | 6 |
| Strings | 6 |
| Implementation | 4 |
| Math | 4 |
| Constructive | 2 |
| Geometry | 2 |
| Data Structures | 1 |
| Number Theory | 1 |

### CSES por categoria

```mermaid
pie showData
    title CSES por categoria
    "Introductory Problems" : 16
    "Sorting and Search" : 8
    "Tree Algorithms" : 4
    "Graph Algorithms" : 3
    "Mathematics" : 3
    "Bitwise Operations" : 1
    "Geometry" : 1
    "Range Queries" : 1
```

### Provas & competições

| Competição | Problemas |
|---|---|
| Mineira 2026 | 8 |
| Paulista 2025 | 6 |
| Paulista 2026 | 6 |
| ICPC 2020 | 5 |
| ICPC 2013 | 4 |
| ICPC FP 2024 | 3 |
| ICPC Summer School 2026 | 3 |
| Phase Zero 2025 | 2 |
| Zhili Cup 2026 | 2 |
| ICPC FP 2025 | 1 |
<!-- STATS:END -->

---

## 🗂️ Organização do repositório

- **`Codeforces/`** — problemas do Codeforces, organizados **por tema** (Greedy,
  DP, Strings, Geometry, …). Cada arquivo traz no topo um header com o nome do
  problema, código, rating, link e tags.
- **`CSES/`** — problemas do *CSES Problem Set*, por categoria.
- **Pastas de prova** (`Paulista-2026/`, `ICPC-FP-2024/`, `Mineira-2026/`, …) —
  soluções de competições, mantidas com o nome original do problema.

> As estatísticas e gráficos acima são gerados por
> [`scripts/gen_readme_stats.py`](scripts/gen_readme_stats.py). Para atualizar
> localmente: `python3 scripts/gen_readme_stats.py`.
